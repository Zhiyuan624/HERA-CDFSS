...
class AverageMeter:
    ...
    def __init__(self, dataset, device):
        ...
        self.class_ids_interest = torch.tensor(dataset.class_ids).to(self.device)
        ...
        self.intersection_buf = torch.zeros([2, self.nclass], dtype=torch.float32).to(self.device)
        self.union_buf = torch.zeros([2, self.nclass], dtype=torch.float32).to(self.device)
...
class Logger:
    ...
    @classmethod
    def initialize(cls, logdir='./logs', logfile='log.txt'):
        cls.logpath = logdir
        os.makedirs(cls.logpath, exist_ok=True)

        log_file_path = os.path.join(cls.logpath, logfile)
