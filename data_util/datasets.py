...
class FSSDataset:
    ...
    @classmethod
    def build_dataloader(cls, benchmark, bsz, nworker, split, shot=1, fold=0):
        # Force randomness during training for diverse episode combinations
        # Freeze randomness during testing for reproducibility
        shuffle = split == 'trn'
        nworker = nworker if split == 'trn' else 0
        dataset_class = cls.datasets[benchmark]

        if benchmark == 'pascal':
            dataset = dataset_class(cls.datapath, transform=cls.transform, split=split, shot=shot, fold=fold)
        else:
            dataset = dataset_class(cls.datapath, transform=cls.transform, split=split, shot=shot)
