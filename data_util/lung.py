...
class DatasetLung(Dataset):
    def __init__(self, datapath, transform, split, shot, num=600):
        ...
        cand1 = os.path.join(datapath, 'LungSegmentation')
        cand2 = os.path.join(datapath, 'Lung Segmentation')  # 你现在的
        if os.path.isdir(cand1):
            self.base_path = cand1
        elif os.path.isdir(cand2):
            self.base_path = cand2
        else:
            raise FileNotFoundError(f"Cannot find LungSegmentation folder under {datapath}")
    ...
    def build_img_metadata_classwise(self):
        ...
        for cat in self.categories:
            img_paths = sorted(glob.glob(os.path.join(self.ann_path, '**', '*.png'), recursive=True))
            for img_path in img_paths:
                if os.path.basename(img_path).split('.')[1] == 'png':
